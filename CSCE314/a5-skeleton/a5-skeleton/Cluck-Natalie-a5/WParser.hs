...
module WParser ( parse,
  ...
  factor = (nat >>= \n -> return $ Val (VInt n))
		+++ stringLiteral +++ intLiteral +++ parens expr

  factorSeq left =
	( do op <- (symbol "*" >> return Multiplies) +++ (symbol "/" >> return Divides)
	     right <- factor
  ...
  boolSeq left =
    ( do op <- (symbol "=" >> return Equals) +++ (symbol "!=" >> return NotEqual) +++ (symbol "<" >> return Less) +++ (symbol ">" >> return Greater) +++ (symbol "<=" >> return LessOrEqual) +++ (symbol ">=" >> return GreaterOrEqual)
         right <- state
         termSeq (op left right)
    ) +++ return left
